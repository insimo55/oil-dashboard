...
export const WellCard: React.FC<{ well: Well }> = ({ well }) => {
  ...
  return (
    ...
    <Link href={`/wells/${well.id}`} className="block h-full">
        <div className={`bg-white dark:bg-neutral-900/60 backdrop-blur-sm rounded-xl shadow-lg p-5 border-l-4 ${statusColor} transition-transform duration-300 hover:scale-105` }>
          ...
          <div className="mb-4">
            <div className="flex justify-between items-baseline mb-1 text-sm">
              <span className="font-semibold text-gray-700 dark:text-gray-300">Прогресс:</span>
              <span className="text-gray-600 dark:text-gray-300 font-mono">{well.current_depth}м / {well.planned_depth}м</span>
            </div>
          ...
          {/* Информация блоками */}
            ...
            <div className="space-y-3 text-sm">
              <div className="flex justify-between">
                <span className="text-gray-500 dark:text-gray-400">Секция:</span>
                <span className="font-semibold text-gray-800 dark:text-gray-100">{well.current_section_display}</span>
              </div>
            </div>
            {/* НОВЫЙ БЛОК ДЛЯ ИНЖЕНЕРОВ */}
            {engineerList.length > 0 && (
              <div className="mt-4 pt-4 border-t border-gray-200/80 dark:border-neutral-800">
        ...
        <div className="mt-auto pt-4 border-t border-gray-200/80 dark:border-neutral-800">
